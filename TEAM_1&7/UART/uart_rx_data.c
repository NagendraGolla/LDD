...
static ssize_t uart_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos) {
    ...
    int i=0,value=0;
     ...
     if (copy_from_user(tx, buf, count))  // Copy a byte of data from user space
            return -EFAULT;  // Return error if copy fails
      value=simple_strtol(tx,NULL,10);
      if(value==0 || value==1)
    ...
    while (count--) {
        ...
        writel(tx+i, uart_base + UART_DR); // Write the character to the data register
    	i++;
    }
...
static ssize_t uart_read(struct file *file, char __user *buf, size_t count, loff_t *ppos) {
    ...
    data[i] = readl(uart_base + UART_DR) & 0xFF; // Read a byte from the UART data register
    if(data[i]=='\0')
	    break;
	    i++;
    }
    if (copy_to_user(buf, data, i)) // Copy the byte to user space
        return -EFAULT; // Return error if copy fails
    if(c=='\0')
	    break;
